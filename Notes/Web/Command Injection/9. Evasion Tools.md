...
```shell-session
hellopriyanshu2702@htb[/htb]$ git clone https://github.com/Bashfuscator/Bashfuscator
hellopriyanshu2702@htb[/htb]$ cd Bashfuscator
hellopriyanshu2702@htb[/htb]$ pip3 install setuptools==65
hellopriyanshu2702@htb[/htb]$ python3 setup.py install --user
```
...
```shell-session
hellopriyanshu2702@htb[/htb]$ cd ./bashfuscator/bin/
hellopriyanshu2702@htb[/htb]$ ./bashfuscator -h
...
```shell-session
hellopriyanshu2702@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd'
...
```shell-session
hellopriyanshu2702@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd' -s 1 -t 1 --no-mangling --layers 1
...
```shell-session
hellopriyanshu2702@htb[/htb]$ bash -c 'eval "$(W0=(w \  t e c p s a \/ d);for Ll in 4 7 2 1 8 3 2 4 8 5 7 6 6 0 9;{ printf %s "${W0[$Ll]}";};)"'
