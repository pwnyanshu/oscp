...
A `directory` is a `hierarchical data store` that contains information about network resources such as 
- `users`, 
- `groups`, 
- `computers`, 
- `printers`, 
...
There are two popular implementations of LDAP 
- `OpenLDAP`, an open-source software widely used and supported,  
- `Microsoft Active Directory`, a Windows-based implementation that seamlessly integrates with other Microsoft products and services.
...
LDAP works by using a `client-server architecture`. A client sends an LDAP request to a server, which searches the directory service and returns a response to the client. LDAP supports various requests, such as `bind`, `unbind`, `search`, `compare`, `add`, `delete`, `modify`, etc.
...
![[Pasted image 20260224201209.png]]

---

## Enumeration

![[Pasted image 20260224201807.png]]
