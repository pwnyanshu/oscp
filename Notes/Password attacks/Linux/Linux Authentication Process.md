Linux-based distributions support various authentication mechanisms. One of the most commonly used is [Pluggable Authentication Modules (PAM)](https://web.archive.org/web/20220622215926/http://www.linux-pam.org/Linux-PAM-html/Linux-PAM_SAG.html). 
...
These fields are separated by colons (`:`). As such, a typical entry may look something like this:
...
In the password field usually, we will find the value `x` in this field, indicating that the passwords are stored in a hashed form within the `/etc/shadow` file. 

 In rare cases (generally on very old systems) this field may hold the actual password hash.
...
The `/etc/shadow` file is also only readable by users with administrative privileges.
...
```
sudo cat /etc/security/opasswd cry0l1t3:1000:2:$1$HjFAfYTG$qNDkF0zJ3v8ylCOrKB0kt0,$1$kcUjWZJX$E9uMSmiQeRh4pAAgzuvkq1
````
...
john unshadowed.hash --single
````

**Note:** This is the exact scenario that JtR's `single crack mode` was designed for.

![[Pasted image 20260308024039.png]]

![[Pasted image 20260308025228.png]]
